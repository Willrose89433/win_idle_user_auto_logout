...
This script with a scheduled task, that runs however its configured, logs off any idle user that has been signed on and idle for 3 or more hours
...
3. Move the script somewhere permanent, e.g. C:\Scripts\IdleLogoff.ps1, then register the scheduled task yourself (elevated PowerShell):
...
$trigger.Repetition = (New-ScheduledTaskTrigger -Once -At 8:00PM -RepetitionInterval (New-TimeSpan -Minutes 30) -RepetitionDuration (New-TimeSpan -Hours 4)).Repetition
...
This runs the check every 30 minutes from 8:00 PM to midnight, as SYSTEM (needed to log off other users' sessions), regardless of who's logged in. Adjust -RepetitionDuration if you want it to keep checking past midnight.
